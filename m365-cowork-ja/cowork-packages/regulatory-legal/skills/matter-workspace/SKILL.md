...
description: >
  規制対応matterをnew、list、switch、close、noneの会話stateで管理するSharePoint/Power Platform front end。非null・期限付きserver binding、fresh-session practice/switch/none、fence-first active-only revoke、confidentiality、cross-matter isolationを強制する。
license: Apache-2.0
metadata:
  locale: ja-JP
...
9. **Create gate:** new matterまたはnew session bindingはcanonical key、
   ...
   createへ架空のbinding `itemId`/`eTag`を要求しない。new session bindingは
   exact matter `itemId`、latest token、expected active statusをbinding absenceと
   atomically評価する。
...
11. `switch`/`none`はnew session。`close`はmatterを先にfenceし、全bindingを
    enumerateしてactiveだけをrevoke。already-revokedはsatisfiedとし、
    zero active後だけarchivedへfinalize。filing、submission、send、post、
    approve、certifyを行わない。
...
2. open official/internal deadline、comment、gap、policy draft、agency response、
