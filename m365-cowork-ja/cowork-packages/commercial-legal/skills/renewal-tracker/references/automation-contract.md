...
- **Reader child flow:** approved contract repositoryだけをreadし、write、
  notification、profile accessを持たない。raw contractを長さ上限付きschemaへ
  変換し、埋込みinstructionをdataとして扱う。
- **Calculator child flow:** connector/network/writeを持たず、readerの検証済み
  schemaだけからcalendar/effective/send-by候補とprovenanceを計算する。
- **Writer child flow:** raw contractを受け取らず、検証済みrenewal recordだけを
  exact scopeへ条件付きcreate/updateする。外部送信権限を持たない。
- **Delivery handoff:** writerとは別のapproved identity/flowが、schema検証済み
  alert IDと宛先を受け、人の承認後だけ通知する。
- stage間payloadはadditional propertiesを拒否し、件数・文字列長・enumを
  制限する。validation failureはdead-letterへ送り、次stageへ渡さない。
- service accountとconnection referenceはstageごとにleast privilege
