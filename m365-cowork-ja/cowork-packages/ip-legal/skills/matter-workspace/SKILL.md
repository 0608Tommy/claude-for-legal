...
description: >
  IP matterを`new`、`list`、`switch`、`close`、`none`の会話stateで管理するSharePoint/Power Platform front end。非null・期限付きserver binding、fresh-session switch/none、close時の全binding revoke、conflict・clean-team・未公開発明の分離を強制する。
license: Apache-2.0
metadata:
  locale: ja-JP
...
3. open deadline、C&D/takedown draft、Customs/platform route、filing/renewal、
